...
def animated_plot_piaskownica():
    ...
    theta = np.arange(0, np.pi * 2, 0.0001)
    alfa = np.arange(0, np.pi * 2, 0.001)
    ax.plot(R_Z * np.cos(theta), R_Z * np.sin(theta), lw=2, color='b',
             label='Ziemia')  # model ziemi
    ax.plot((R_Z + 85000) * np.cos(alfa), (R_Z + 85000) * np.sin(alfa), color='y', lw=0.3,
             label='Koniec atmosfery')
    ax.text(n, 0.2 * R_Z, napis_prędkości, fontsize=10)
    ax.text(n, 0.1 * R_Z, napis_paliwa, fontsize=10)
    ax.text(n, 0.0 * R_Z, napis_gazów, fontsize=10)
    plt.legend()
    plt.xlabel('oś X')
    plt.ylabel('oś Y')

    line, = ax.plot([], [], color='g', animated=True)
    ...
    oX1, = ax1.plot([], [], color='g', animated=True)
    oY1, = ax1.plot([], [], color='r', lw=0.5, animated=True)
    ...
    dx_forplot_animated, dy_forplot_animated, droga_l_animatedx, droga_l_animatedy = [], [], [], []
    dzielnik_punktow = 1

    for q in range(len(x_forplot)):
        if q % dzielnik_punktow == 0:
            dx_forplot_animated.append(x_forplot[q])
            dy_forplot_animated.append(y_forplot[q])
            droga_l_animatedx.append(droga_l[q])
            droga_l_animatedy.append(droga_l[q])

    def rzut(t):
        global test0, test01, test02, test03
        try:
            x_data1.append(dx_forplot_animated[t])
            y_data1.append(droga_l_animatedx[t])
        except:
            pass

        try:
            x_data11.append(dy_forplot_animated[t])
            y_data11.append(droga_l_animatedy[t])
        except:
            pass

        if test0 == 0:
            if test01 == 0:
                if droga_l_animatedx[t] > droga1:
                    test01 = 1
                    try:
                        plt.scatter(wazny_x[0], wazny_y[0],
                                    label='Odpalenie silników')  # początak i start działania silników
            ...
            pass
        oX1.set_data(x_data1, y_data1)
        oY1.set_data(x_data11, y_data11)
        return oX1, oY1
    ...
    anim = matplotlib.animation.FuncAnimation(fg1, rzut, init_func=rzut1, blit=True, interval=2)
...
def animowane_wykresy():
    ...
    oX1, = ax1.plot([], [], color='g', animated=True)
    oY1, = fig1.plot([], [], color='r', lw=0.5, animated=True)
    ...
    anim = matplotlib.animation.FuncAnimation(oX1, rzut, init_func=rzut1, blit=True, interval=2)
