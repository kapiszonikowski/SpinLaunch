...
def obliczanie_przyspieszenia(a,faza):
    global R_xy, X_location, Y_location, v_y, m_r, a_x, a_y, droga, Hnpm, a_t_x, a_t_y, a_x, jak_często
    v_x=a
    if faza == 0:
        if v_x > 0:
...
def updates_appends(a, faza):
    global R_xy, jak_często, X_location, Y_location, v_y, m_r, droga, Hnpm, i, x_forplot, x1_forplot, y_forplot, y1_forplot, droga_l, listH_xy, listH_xy1, listR_xy, v_xy_l, v_x_l, v_y_l, a_xy_l, a_x_l, a_y_l
    v_x=a
    if faza == 0:
...
def naprowadzanie_na_orbite(a):
    global R_xy, X_location, Y_location, v_y, m_r, a_x, a_y, droga, Hnpm, a_t_x, a_t_y, a_x, test
    v_x = a
...
def obliczenia_numeryczne(a, faza, p_important_values, wysokość, tryb_pracy_silników):
    global R_xy, X_location, Y_location, m_r, a_x, a_y, droga, Hnpm, a_t_x, a_t_y, a_x, H_startu, jak_często, test
    v_x=a
    if faza == 0:
        while R_xy > R_Z:
            obliczanie_przyspieszenia(a, faza)

            updates_appends(a, faza)
...
def reset_xyv(a):
    global v_y, X_location, Y_location, R_xy, v_x0, v_y0, x_0, y_0, Hnpm, R_Z
    v_x=a
    v_x = v_x0
    v_y = v_y0
    X_location = x_0
...
def wyświetlanie_wykresów(orbita, dane, spin):
    if orbita == 1:
        ...
        plt.plot((R_Z+23.3) * np.cos(theta), (R_Z+23.3) * np.sin(theta), lw=2, color='b', label='Ziemia')  # model ziemi
...
if spin == 1:
    ...
    obliczenia_numeryczne(a,0, 0, wysokość=H_startu, tryb_pracy_silników=0)  # Silniki nie zadzaiłały

    reset_xyv(a)  # resetuje wartości po I przelocie
    print('a')

    obliczenia_numeryczne(a,1, 1, wysokość=H_startu, tryb_pracy_silników=0)  # I faza ruchu - do odpalenia silników
    ...
    obliczenia_numeryczne(a, 2, 1, wysokość=H_startu, tryb_pracy_silników=0)  # II faza ruchu - z silnikami
    print('a')
    ...
    if test == 1:
        ...
        update_important_values(x_forplot[-1], y_forplot[-1], droga_l[-1], listH_xy[-1], a_xy_l[-1], v_xy_l[-1])
        test = 0
    obliczenia_numeryczne(a,3, 0, wysokość=H_startu, tryb_pracy_silników=0)  # III faza ruchu - po wyczerpaniu paliwa
